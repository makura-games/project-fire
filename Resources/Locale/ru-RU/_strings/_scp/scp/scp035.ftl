scp035-protection-success = Ваше сознание сопротивляется прикасаться к { $name }

scp-035-ghost-role-name = Носитель Scp-035
scp-035-ghost-role-desc = Вы обрели тело, время выбираться отсюда.

scp-035-message-one = Ну же, надень меня!
scp-035-message-two = Чего ты ждешь! Надень меня..
scp-035-message-three = Помоги мне!..

scp-035-max-servants = Достигнуто максимальное число!
scp-035-reject-you = Маска отвергает вас!
scp-035-reject-target = Работает только на людей.
scp-035-stun-effect = Ваше тело онемело!
scp-035-paralyze-effect = Вы ошеломлены!

scp-035-action-stun = Вам стоит прилечь.
